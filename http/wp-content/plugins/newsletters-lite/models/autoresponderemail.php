...
class wpmlAutoresponderemail extends wpmlDbHelper {
	...
	function validate($data = array()) {
		...
		if (!empty($data)) {
			if (empty($autoresponder_id)) { $this -> errors['autoresponder_id'] = __('No autoresponder was specified.', $this -> plugin_name); }
		...
		if (empty($this -> errors)) {
			...
			if ($Db -> find(array('autoresponder_id' => $autoresponder_id, 'subscriber_id' => $subscriber_id))) {
				$this -> errors[] = __('Autoresponder email already exists.', $this -> plugin_name);	
			}
		}
