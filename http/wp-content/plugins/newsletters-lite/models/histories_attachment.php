...
class wpmlHistoriesAttachment extends wpMailPlugin {
	...
	function validate($data = array()) {
		...
		if (empty($title)) { $this -> errors['title'] = __('Please fill in an attachment title.', $this -> plugin_name); }
		if (empty($history_id)) { $this -> errors['history_id'] = __('No history record was specified.', $this -> plugin_name); }
		if (empty($filename)) { $this -> errors['filename'] = __('No mailing list was specified', $this -> plugin_name); }
