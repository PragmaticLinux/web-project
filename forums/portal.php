...
if (!empty($mybb->settings['portal_announcementsfid'])) {
    ...
    if (!empty($posts)) {
        ...
        $query = $db->query("
			SELECT t.*, t.username AS threadusername, u.username, u.avatar, u.avatardimensions,t.views
			FROM " . TABLE_PREFIX . "threads t
			LEFT JOIN " . TABLE_PREFIX . "users u ON (u.uid = t.uid)
        ...
        while ($announcement = $db->fetch_array($query)) {
            ...
            $post['attachments'] = '';
            if ($mybb->settings['enableattachments'] == 1 && isset($attachcache[$announcement['pid']]) && is_array($attachcache[$announcement['pid']])) { // This post has 1 or more attachments
                $validationcount = 0;
                $id = $announcement['pid'];
